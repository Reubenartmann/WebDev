    ...
    constructor(props) {
      ...
      initBoard() {
        ...
        for (let tempRow = 0; tempRow < 3; tempRow++) {
          ...
          console.log("Init RowConter: " + rowcounter)
        }
        this.setState({
          board,
      ...
      componentDidMount() {
        ...
        brRef.on('value', (snapshot) => {
          ...
          if (!this.state.gameStatus && this.state.pAccountId !== this.state.mostRecentTurn.pAccountId){
          this.place(newState.column, newState.row)
          }
          this.setState({
            mostRecentTurn: newState
          });
        });
      }
      togglePlayer() {
      ...
      place(tempColumn, tempRow) {
        if (!this.state.gameStatus && this.state.pAccountId != '' && this.state.pAccountId !== this.state.mostRecentTurn.pAccountId) {
          console.log(this.state.mostRecentTurn.pAccountId + " " + this.state.pAccountId)
          let board = this.state.board;
            if (!board[tempRow][tempColumn]) {
              board[tempRow][tempColumn] = this.state.currentP;
              this.state.column = tempColumn;
